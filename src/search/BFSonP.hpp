...
class BFSonP : public BoundedSuboptimalBase<Domain, Node>
{
...
public:
    BFSonP(Domain& domain_, const string& sorting_)
    ...
    double run(SearchResultContainer& res)
    {
        ...
        focal.swapComparator(Node::compareNodesOnProb);
        ...
        while (!open.empty()) {
            ...
            if (fmin != fminNode->getFValue()) {
                ...
                //cerr << "fmin changes, re-order focal\n";

                // re-order the focal because bound change cause the prob change
                for (auto& nodePtr : focal) {
                    nodePtr->computeExpectedEffortValue(fhatminNode,
                                                        fhatminVar);
                }

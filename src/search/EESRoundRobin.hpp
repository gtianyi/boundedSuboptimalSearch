...
class EESRoundRobin : public BoundedSuboptimalBase<Domain, Node>
{
...
public:
    EESRoundRobin(Domain& domain_, const string& sorting_, int dExp_)
...
private:
    ...
    {
        auto curNum = static_cast<int>(expansion) % (dExp + 1 + 1);
        if (curNum <= dExp) {
            return Qtype::focal;
        }
        if (curNum == dExp + 1) {
    ...
    Node* selectNode(size_t expansionNum, Qtype& nodeFrom)
    {
        ...
        if (qtype == Qtype::open && !open.empty()) {
